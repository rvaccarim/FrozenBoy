    ...
    public class OpcodeHandler {
        ...
        private Dictionary<u8, Opcode> InitializeOpcodes() {
            return new Dictionary<u8, Opcode> {
                ...
                { 0xCE, new Opcode(0xCE, "ADC A, ${0:x2}",       2,  8, [() => { ADC(mmu.Read8(regs.PC++)); }])},  // no need to split it in two steps,
                ...
                { 0xC6, new Opcode(0xC6, "ADD A, ${0:x2}",       2,  8, [() => { ADD(mmu.Read8(regs.PC++)); }])},

                { 0xE8, new Opcode(0xE8, "ADD SP, ${0:x2}",      2, 16, [() => { operand8 = mmu.Read8(regs.PC++); },
                ...
                { 0x34, new Opcode(0x34, "INC (HL)",             1, 12, [() => { operand8 = mmu.Read8(regs.HL); },
                                                                         () => { mmu.Write8(regs.HL, INC(operand8)); },])},
                // INC - 16 bit
                { 0x03, new Opcode(0x03, "INC BC",               1,  8, [() => { if (OAM_Bug(regs.BC)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.BC++; } ])},
                { 0x13, new Opcode(0x13, "INC DE",               1,  8, [() => { if (OAM_Bug(regs.DE)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.DE++; } ])},
                { 0x23, new Opcode(0x23, "INC HL",               1,  8, [() => { if (OAM_Bug(regs.HL)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.HL++; } ])},
                { 0x33, new Opcode(0x33, "INC SP",               1,  8, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.SP++; } ])}, 
                ...
                { 0xE6, new Opcode(0xE6, "AND ${0:x2}",          2,  8, [() => { AND(mmu.Read8(regs.PC++)); }])},
                // OR
                ...
                { 0xF6, new Opcode(0xF6, "OR ${0:x2}",           2,  8, [() => { OR(mmu.Read8(regs.PC++)); }])},
                // XOR
                ...
                { 0xEE, new Opcode(0xEE, "XOR ${0:x2}",          2,  8, [() => { XOR(mmu.Read8(regs.PC++)); }])},

                // ==================================================================================================================
                // CALL AND RETURN
                // ==================================================================================================================
                // Push address of next instruction onto stack and then jump to address nn.
                { 0xCD, new Opcode(0xCD, "CALL ${0:x4}",         3, 24, [() => { lsb = mmu.Read8(regs.PC++); },
                ...
                { 0xC4, new Opcode(0xC4, "CALL NZ, ${0:x4}",     3, 24, [() => { lsb = mmu.Read8(regs.PC++); }, //  8t
                ...
                { 0xCC, new Opcode(0xCC, "CALL Z, ${0:x4}",      3, 24, [() => { lsb = mmu.Read8(regs.PC++); }, //  8t
                                                                         () => { msb = mmu.Read8(regs.PC++); if (!regs.FlagZ) { stop = true; } }, // 12t
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.PUSH_1);
                                                                                 regs.SP--; mmu.Write8(regs.SP, BitUtils.Msb(regs.PC)); }, // 16t
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.PUSH_2);
                ...
                { 0xD4, new Opcode(0xD4, "CALL NC, ${0:x4}",     3, 24, [() => { lsb = mmu.Read8(regs.PC++); }, //  8t
                ...
                { 0xDC, new Opcode(0xDC, "CALL C, ${0:x4}",      3, 24, [() => { lsb = mmu.Read8(regs.PC++); }, //  8t
                                                                         () => { msb = mmu.Read8(regs.PC++); if (!regs.FlagC) { stop = true; } }, // 12t
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.PUSH_1);
                                                                                 regs.SP--; mmu.Write8(regs.SP, BitUtils.Msb(regs.PC)); }, // 16t
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.PUSH_2);
                ...
                { 0xC0, new Opcode(0xC0, "RET NZ",               1, 20, [// You can't pass a value by ref to an anonymous function
                ...
                { 0xC8, new Opcode(0xC8, "RET Z",                1, 20, [() => { if (! regs.FlagZ) {stop = true; } },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++; },
                ...
                { 0xD0, new Opcode(0xD0, "RET NC",               1, 20, [() => { if (regs.FlagC) {stop = true; } },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++; },
                ...
                { 0xD8, new Opcode(0xD8, "RET C",                1, 20, [() => { if (! regs.FlagC) {stop = true; } },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++; },
                ...
                { 0xC9, new Opcode(0xC9, "RET",                  1, 16, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0xD9, new Opcode(0xD9, "RETI",                 1, 16, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++; },
                ...
                { 0xFE, new Opcode(0xFE, "CP ${0:x2}",           2,  8, [() => { CP(mmu.Read8(regs.PC++)); }])},

                // ==================================================================================================================
                // INTERRUPTS
                // ==================================================================================================================
                // Disables interrupt handling by setting IME=0 and cancelling any scheduled effects of the EI instruction if any
                { 0xF3, new Opcode(0xF3, "DI",                   1,  4, [() => { intManager.DisableInterrupts(); }])},
                // Schedules interrupt handling to be enabled after the next machine cycle
                { 0xFB, new Opcode(0xFB, "EI",                   1,  4, [() => { intManager.EnableInterrupts(true); }])},
                                        
                // ==================================================================================================================
                // JUMP FAMILY
                // ==================================================================================================================
                // JP - Jump to location
                { 0xE9, new Opcode(0xE9, "JP (HL)",              1,  4, [() => { regs.PC = regs.HL; }])},

                { 0xC3, new Opcode(0xC3, "JP ${0:x4}",           3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); } ])},

                { 0xC2, new Opcode(0xC2, "JP NZ, ${0:x4}",       3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); if (regs.FlagZ) { stop=true; } },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); } ])},

                { 0xCA, new Opcode(0xCA, "JP Z, ${0:x4}",        3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); if (! regs.FlagZ) { stop=true; } },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); } ])},

                { 0xD2, new Opcode(0xD2, "JP NC, ${0:x4}",       3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); if (regs.FlagC) { stop=true; } },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); } ])},

                { 0xDA, new Opcode(0xDA, "JP C, ${0:x4}",        3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); if (! regs.FlagC) { stop=true; } },
                                                                         () => { regs.PC = BitUtils.ToUnsigned16(msb, lsb); } ])},
                // Jump to location relative to the current location
                // 12t if branched, 8 if not branched 
                { 0x18, new Opcode(0x18, "JR ${0:x2}",           2, 12, [() => { operand8 = mmu.Read8(regs.PC++); },
                                                                         () => { regs.PC = (u16)(regs.PC + ToSigned(operand8)); } ])},

                { 0x20, new Opcode(0x20, "JR NZ, ${0:x2}",       2, 12, [() => { operand8 = mmu.Read8(regs.PC++); if ( regs.FlagZ) { stop = true; } },
                                                                         () => { regs.PC = (u16)(regs.PC + ToSigned(operand8)); } ])},

                { 0x28, new Opcode(0x28, "JR Z, ${0:x2}",        2, 12, [() => { operand8 = mmu.Read8(regs.PC++); if (!regs.FlagZ) { stop = true; } },
                                                                         () => { regs.PC = (u16)(regs.PC + ToSigned(operand8));  } ])},

                { 0x30, new Opcode(0x30, "JR NC, ${0:x2}",       2, 12, [() => { operand8 = mmu.Read8(regs.PC++); if ( regs.FlagC) { stop = true; } },
                                                                         () => { regs.PC = (u16)(regs.PC + ToSigned(operand8));  } ])},

                { 0x38, new Opcode(0x38, "JR C, ${0:x2}",        2, 12, [() => { operand8 = mmu.Read8(regs.PC++); if (!regs.FlagC) { stop = true; } },
                                                                         () => { regs.PC = (u16)(regs.PC + ToSigned(operand8)); } ])},
                ...
                { 0x06, new Opcode(0x06, "LD B, ${0:x2}",        2,  8, [() => { regs.B = mmu.Read8(regs.PC++); }])},
                { 0x0E, new Opcode(0x0E, "LD C, ${0:x2}",        2,  8, [() => { regs.C = mmu.Read8(regs.PC++); }])},
                { 0x16, new Opcode(0x16, "LD D, ${0:x2}",        2,  8, [() => { regs.D = mmu.Read8(regs.PC++); }])},
                { 0x1E, new Opcode(0x1E, "LD E, ${0:x2}",        2,  8, [() => { regs.E = mmu.Read8(regs.PC++); }])},
                { 0x26, new Opcode(0x26, "LD H, ${0:x2}",        2,  8, [() => { regs.H = mmu.Read8(regs.PC++); }])},
                { 0x2E, new Opcode(0x2E, "LD L, ${0:x2}",        2,  8, [() => { regs.L = mmu.Read8(regs.PC++); }])},
                { 0x3E, new Opcode(0x3E, "LD A, ${0:x2}",        2,  8, [() => { regs.A = mmu.Read8(regs.PC++); }])},
                { 0x36, new Opcode(0x36, "LD (HL), ${0:x2}",     2, 12, [() => { operand8 = mmu.Read8(regs.PC++); },
                                                                         () => { mmu.Write8(regs.HL, operand8); }, ])},
                // load direct value into register - 16 bit
                { 0x01, new Opcode(0x01, "LD BC, ${0:x4}",       3, 12, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); regs.BC = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0x11, new Opcode(0x11, "LD DE, ${0:x4}",       3, 12, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); regs.DE = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0x21, new Opcode(0x21, "LD HL, ${0:x4}",       3, 12, [() => { if (OAM_Bug(regs.PC)) CorruptOAM(CorruptionType.LD_HL);
                                                                                     lsb = mmu.Read8(regs.PC++); },
                                                                         () => { if (OAM_Bug(regs.PC)) CorruptOAM(CorruptionType.LD_HL);
                                                                                     msb = mmu.Read8(regs.PC++);
                                                                                     regs.HL = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0x31, new Opcode(0x31, "LD SP, ${0:x4}",       3, 12, [() => { lsb = mmu.Read8(regs.PC++); },
                ...
                { 0x08, new Opcode(0x08, "LD (${0:x4}), SP",     3, 20, [() => { lsb = mmu.Read8(regs.PC++); },
                ...
                { 0x2A, new Opcode(0x2A, "LD A, (HL+)",          1,  8, [() => { if (OAM_Bug(regs.HL)) CorruptOAM(CorruptionType.LD_HL);
                                                                                 regs.A = mmu.Read8(regs.HL); regs.HL++; } ])},
                { 0x3A, new Opcode(0x3A, "LD A, (HL-)",          1,  8, [() => { if (OAM_Bug(regs.HL)) CorruptOAM(CorruptionType.LD_HL);
                                                                                 regs.A = mmu.Read8(regs.HL); regs.HL--; } ])},
                ...
                { 0xF8, new Opcode(0xF8, "LD HL, SP+${0:x2}",    2, 12, [() => { operand8 = mmu.Read8(regs.PC++); },
                                                                         () => { regs.HL = ADD_Signed8(regs.SP, operand8); }, ])},

                { 0xF9, new Opcode(0xF9, "LD SP, HL",            1,  8, [() => { regs.SP = regs.HL; }])},

                { 0xEA, new Opcode(0xEA, "LD (${0:x4}), A",      3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); },
                                                                         () => { mmu.Write8(BitUtils.ToUnsigned16(msb, lsb), regs.A); }, ])},

                { 0xFA, new Opcode(0xFA, "LD A, (${0:x4})",      3, 16, [() => { lsb = mmu.Read8(regs.PC++); },
                                                                         () => { msb = mmu.Read8(regs.PC++); },
                                                                         () => { regs.A = mmu.Read8(BitUtils.ToUnsigned16(msb, lsb)); }, ])},                                                                 
                // LDH - Put memory address $FF00+n into A
                { 0xE0, new Opcode(0xE0, "LDH (${0:x2}), A",     2, 12, [() => { operand8 = mmu.Read8(regs.PC++); },
                                                                         () => { mmu.Write8((u16)(0xFF00 + operand8), regs.A); }, ])},
                
                { 0xF0, new Opcode(0xF0, "LDH A, (${0:x2})",     2, 12, [() => { operand8 = mmu.Read8(regs.PC++); },
                ...
                // ==================================================================================================================
                // Pop
                { 0xC1, new Opcode(0xC1, "POP BC",               1, 12, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++;
                                                                                 regs.BC = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0xD1, new Opcode(0xD1, "POP DE",               1, 12, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++;
                                                                               },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++;
                ...
                { 0xE1, new Opcode(0xE1, "POP HL",               1, 12, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++;
                                                                                 regs.HL = BitUtils.ToUnsigned16(msb, lsb); }, ])},

                { 0xF1, new Opcode(0xF1, "POP AF",               1, 12, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_1);
                                                                                 lsb = mmu.Read8(regs.SP); regs.SP++; },
                                                                         () => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.POP_2);
                                                                                 msb = mmu.Read8(regs.SP); regs.SP++;
                ...
                { 0x35, new Opcode(0x35, "DEC (HL)",             1, 12, [() => { operand8 = mmu.Read8(regs.HL); },
                                                                         () => { mmu.Write8(regs.HL, DEC(operand8)); }, ])},
                // DEC 16 bit                                  
                { 0x0B, new Opcode(0x0B, "DEC BC",               1,  8, [() => { if (OAM_Bug(regs.BC)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.BC--; } ])},
                { 0x1B, new Opcode(0x1B, "DEC DE",               1,  8, [() => { if (OAM_Bug(regs.DE)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.DE--; } ])},
                { 0x2B, new Opcode(0x2B, "DEC HL",               1,  8, [() => { if (OAM_Bug(regs.HL)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.HL--; } ])},
                { 0x3B, new Opcode(0x3B, "DEC SP",               1,  8, [() => { if (OAM_Bug(regs.SP)) CorruptOAM(CorruptionType.INC_DEC);
                                                                                 regs.SP--; } ])},
                ...
                { 0xDE, new Opcode(0xDE, "SBC A, ${0:x2}",       2,  8, [() => { SBC(mmu.Read8(regs.PC++)); }])},
                ...
                { 0xD6, new Opcode(0xD6, "SUB ${0:x2}",          2,  8, [() => { SUB(mmu.Read8(regs.PC++)); }])},
        ...
        private static bool HasCarry(int value) {
            return (value >> 8) != 0;
        }
